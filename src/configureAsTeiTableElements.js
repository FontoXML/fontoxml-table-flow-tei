 ...
 * before each row using {@link createIconWidget}. Row widgets are linked to the row elements of the table. Any widget can be added but only icon widget is supported.
 * @param  {WidgetSubAreaByName|Widget[]|null}  [options.columnBefore]   Used to add one or multiple
 * widgets before each column. The context node of each column widget is the cell element in the first row. If
 * there is even one merged cell (to left or right) in the first row, the widgets in columnBefore
 * widget area are not rendered. {@link fonto-documentation/docs/editor/api/index.xml#id-9d2b1ad5-bbc1-6c44-d491-16dc213c53f2 All widgets} are supported.
